...
// MT2W = GetMt2w(*Lep,selJet,selJetB,Nbjet,lv_RecoMET.Px(),lv_RecoMET.Py())
...
void nTupler(const char *inputFile, string outname)
{
  ...
  */
  // From Batool nTuplerCode_Base
  std::vector<TLorentzVector> GenPar;
  ...
  //From Batool nTuplerCode_Base
   tree->Branch("GenParticles",&GenPar);
   tree->Branch("NGen",&NGenPar,"NGenPar/I");
   tree->Branch("GenpdgId",&GenpdgId);
   tree->Branch("MomIndex",&MomIndex);
    ///////////////////////////////////
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry)
  {
    ...
    GenParticle* d1;
    GenParticle* d2;
    GenParticle* Granmom1;
    GenParticle* part;
    GenParticle* mom1;
    NGenPar = branchGP->GetEntries();


    for(int igp = 0; igp < NGenPar; igp++)
      {
        part = (GenParticle*) branchGP->At(igp);
       if(part->Status !=3) continue;
        if(part->M1 != -1)mom1  = (GenParticle*) branchGP->At(part->M1);
        lv_gen.SetPtEtaPhiM(part->PT, part->Eta, part->Phi, part->Mass);
        GenPar.push_back(lv_gen);
        MomIndex.push_back(part->M1);
        GenpdgId.push_back(part->PID);
      }
